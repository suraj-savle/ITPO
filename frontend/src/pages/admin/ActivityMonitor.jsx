...
const ActivityMonitor = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      const [activitiesRes, statusRes] = await Promise.all([
        makeAuthenticatedRequest(`/api/admin/activities${selectedRole ? `?role=${selectedRole}` : ''}`),
        makeAuthenticatedRequest(`/api/admin/users-status${selectedRole ? `?role=${selectedRole}` : ''}`)
      ]);
  ...
  const getRoleColor = (role) => {
    switch (role) {
      ...
      case 'mentor': return 'text-purple-600 bg-purple-50';
      case 'recruiter': return 'text-green-600 bg-green-50';
      default: return 'text-gray-600 bg-gray-50';
    }
  };
    ...
    <div className="max-w-7xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Track user activities and status</p>
        </div>
        <button
          onClick={fetchData}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-white rounded-xl border border-gray-100 p-6">
          <div className="flex items-center justify-between">
            ...
            <span className="text-2xl font-semibold text-gray-900">{statusSummary.active || 0}</span>
          </div>
        ...
        <div className="bg-white rounded-xl border border-gray-100 p-6">
          <div className="flex items-center justify-between">
            ...
            <span className="text-2xl font-semibold text-gray-900">{statusSummary.pending || 0}</span>
          </div>
        ...
        <div className="bg-white rounded-xl border border-gray-100 p-6">
          <div className="flex items-center justify-between">
            ...
            <span className="text-2xl font-semibold text-gray-900">{statusSummary.recentlyActive || 0}</span>
          </div>
                ...
                <div key={user._id} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      {user.assignedMentor && (
                        <p className="text-xs text-purple-600">Mentor: {user.assignedMentor.name}</p>
                      )}
                    </div>
                  </div>
                  <div className="text-right space-y-1">
                    ...
                    <p className="text-sm text-gray-500">Last seen: {getLastSeenText(user.lastLogin)}</p>
                    {user.role === 'student' && (
                      <p className="text-xs text-blue-600">Profile: {user.profileCompletion}%</p>
                    )}
                  </div>
                </div>
              ))
