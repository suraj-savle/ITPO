...
const Applications = () => {
  ...
  const handleWithdraw = async (applicationId) => {
    try {
      ...
      const withdrawnApp = applications.find(
        (app) => app._id === applicationId
      );
      if (withdrawnApp) {
        setWithdrawnApplications((prev) => [
          ...prev,
          { ...withdrawnApp, withdrawnAt: new Date() },
        ]);
      }
  ...
  return (
    <div className="min-h-screen mx-auto p-4 sm:p-6 bg-gradient-to-br from-slate-50 via-indigo-50 to-indigo-100 " >
        ...
        <div className="flex items-center gap-4">
          ...
          >
            {showWithdrawn
              ...
              : `Show Withdrawn (${withdrawnApplications.length})`}
          </button>
          <div className="text-sm text-gray-600">
            {showWithdrawn ? withdrawnApplications.length : applications.length}{" "}
            applications
          </div>
        </div>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {(showWithdrawn ? withdrawnApplications : applications).map(
                (application) => (
