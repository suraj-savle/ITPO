...
import { Clock, CheckCircle, XCircle, User, Briefcase, UserCheck } from 'lucide-react';
...
const History = () => {
  ...
  useEffect(() => {
    const fetchHistory = async () => {
      ...
      try {
        const res = await fetch('http://localhost:5000/api/recruiter/application-history', {
  ...
  return (
    <div className="max-w-6xl mx-auto">
      <div className="mb-6">
        ...
        <p className="text-gray-600">Review all applications you've processed</p>
      </div>

      {applications.length === 0 ? (
        <div className="text-center py-12">
          <p className="text-gray-500">No application history found.</p>
          <p className="text-gray-400 text-sm mt-2">Applications you review will appear here.</p>
        </div>
      ) : (
            ...
            <div key={app._id} className="bg-white rounded-lg shadow p-6">
              ...
              {app.interviewDate && (
                <div className="mt-4 p-3 bg-blue-50 rounded-lg">
                  <p className="text-sm font-medium text-blue-700">Interview Date:</p>
                  <p className="text-sm text-blue-600 mt-1">{new Date(app.interviewDate).toLocaleString()}</p>
                </div>
              )}
              ...
              {app.recruiterNote && (
                <div className="mt-4 p-3 bg-gray-50 rounded-lg">
                  ...
                  <p className="text-sm text-gray-600 mt-1">{app.recruiterNote}</p>
                </div>
              )}
            </div>
          ))}
